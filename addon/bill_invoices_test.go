...
import (
	...
	"github.com/invopop/gobl.br.nfse/addon"
...
func withAddonContext() rules.WithContext {
	return func(rc *rules.Context) {
		rc.Set(rules.ContextKey(addon.V1), tax.AddonForKey(addon.V1))
	}
}
...
func TestSuppliersValidation(t *testing.T) {
	...
	t.Run("validates extensions", func(t *testing.T) {
		...
		sup.Ext = tax.ExtensionsOf(cbc.CodeMap{
			addon.ExtKeySimples:         "1",
			"br-ibge-municipality":     "12345678",
			addon.ExtKeyFiscalIncentive: "2",
		})
		err = rules.Validate(inv, withAddonContext())
		if assert.Error(t, err) {
	...
	for _, ts := range tests {
		t.Run(ts.name, func(t *testing.T) {
			...
			if ts.supplier == nil {
				...
				assert.Equal(t, ts.out, inv.Supplier.Ext.Get(addon.ExtKeyFiscalIncentive))
			}
		})
	}
}
